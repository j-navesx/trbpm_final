...
void carP(car *processing[sizeof(car)],int queue, char *stationsv,int *countstock, stock *stock_ptr[sizeof(stock)]){
  ...
  for(int i=0;i<choice;i++){
    ...
    fscanf(cp,"%c %d %s", &newcar.std,&newcar.fabproc.timeleft,newcar.fabproc.ops);
    }
...
void Search_by_state(car *processing[], int *countProdution_ptr, car *finished[sizeof(car)], int *finish_count) {
  char user_str[20];
  char user;
...
void Search_by_type(car *processing[], int *countProdution_ptr, car *finished[sizeof(car)], int *finish_count) {
  char user_str[20];
  char user;
...
void factory_state_interface(stations station[5], stations_use st[5], car *processing[sizeof(car)], int *countProdution_ptr, car *finished[sizeof(car)], int *finish_count) {
  system("clear");
  char user;
  do {
  ...
  //dá a hipotese do user ir utilizar cada opção do menu que diz factory state (opção 4)
    switch(user) {
      case '1': processes_display(processing,countProdution_ptr);
                break;
      case '2': Search_by_type(processing,countProdution_ptr,finished,finish_count);
                break;
      case '3': Search_by_state(processing,countProdution_ptr,finished,finish_count);
                break;
...
void main(){
  ...
  int queue=0;
  int fin;
  int *finish_count= &fin;
  int count=0;
  int *countProdution_ptr= &count;
  ...
  do {
    ...
    switch(*user) {
      ...
      case '3': carP(processing,queue,stationsv,countstock,stock_ptr);
                system("clear");
                break;
      case '4': factory_state_interface(station,st,processing,countProdution_ptr,finished,finish_count);
                system("clear");
                break;
